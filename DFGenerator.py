...
class DFGenerator(object):
    ...
    def read_special_csv(self, columns=None, seperator='~'):
        if columns:
            return pd.read_csv(self.filename, delimiter=seperator, names=columns, dtype=str, on_bad_lines='skip', engine='python',encoding='latin1')
        else:
            return pd.read_csv(self.filename, delimiter=seperator, dtype=str, on_bad_lines='skip',engine='python',encoding='latin1')
