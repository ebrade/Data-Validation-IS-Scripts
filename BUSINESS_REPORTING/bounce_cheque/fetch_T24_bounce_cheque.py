...
oracle_query = """ 
SELECT RECID,
        EXTRACTVALUE(xmlrecord,'/row/c5[1]/text()') AS BUSINESS_DATE,
        EXTRACTVALUE(xmlrecord,'/row/c5[1]/text()') AS CHEQUE_ISSUED_DATE,
        EXTRACTVALUE(xmlrecord,'/row/c2[1]/text()') AS CURRENCY,
        EXTRACTVALUE(xmlrecord,'/row/c3[1]/text()') AS CHEQUE_AMOUNT_FCY,
        EXTRACTVALUE(xmlrecord,'/row/c31[@m="3"]/text()') AS BOUNCE_CHEQUE_REASON,
        EXTRACTVALUE(xmlrecord,'/row/c4[1]/text()') AS BENEFICIARY
    FROM T24.FBNK_CHEQUE_REGISTE000 WHERE STATUS = 'RETURNED'
"""
